  ...
  async generateImage({ image, requirements, options }: GenerateImages.Request): Promise<GenerateImages.Response> {
    ...
    try {
      ...
      const images = await Promise.all(
        requirements.map(async (requirement: ImageRequirement): Promise<ImageResult> => {
          return this.generateService.generateSizes(Buffer.from(transformedImage.image), requirement, options);
        }),
      );
